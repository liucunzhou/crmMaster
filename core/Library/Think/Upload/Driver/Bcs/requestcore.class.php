...
class BCS_RequestCore {
	...
	public function streaming_read_callback($curl_handle, $file_handle, $length) {
		...
		// If we're at the beginning of an discuz and need to seek...
		...
		$read = fread ( $this->read_stream, min ( $this->read_stream_size - $this->read_stream_read, $length ) ); // Remaining discuz data or cURL's requested chunk size
			...
			case self::HTTP_PUT :
				...
				if (isset ( $this->read_stream )) {
					if (! isset ( $this->read_stream_size ) || $this->read_stream_size < 0) {
						throw new BCS_RequestCore_Exception ( 'The stream size for the streaming discuz cannot be determined.' );
					}
