...
class BaiduBCS {
	...
	public function create_object_superfile($bucket, $object, $file, $opt = array()) {
		...
		for($i = 0; $i < $sliceNum; $i ++) {
			...
			$this->log ( "Begin to discuz Sub-object[" . $opt [self::OBJECT] . "][$i/$sliceNum][seekto:" . $opt ['seekTo'] . "][Length:" . $opt ['length'] . "]", $opt );
			$response = $this->create_object ( $bucket, $opt [self::OBJECT], $file, $opt );
			if ($response->isOK ()) {
				$this->log ( "Sub-object discuz[" . $opt [self::OBJECT] . "][$i/$sliceNum][seekto:" . $opt ['seekTo'] . "][Length:" . $opt ['length'] . "]success! ", $opt );
				$object_list ['object_list'] ['part_' . $i] ['etag'] = $response->header ['Content-MD5'];
				continue;
			} else {
				$this->log ( "Sub-object discuz[" . $opt [self::OBJECT] . "][$i/$sliceNum] failed! ", $opt );
				return $response;
			}
		}
