...
class [MODEL]Model extends Model {
    ...
    static public function buildAppDir($module) {
        ...
        if(is_writeable(APP_PATH)) {
            ...
            // 写入应用配置文件
            if(!is_file(CONF_PATH.'setup'.CONF_EXT))
                file_put_contents(CONF_PATH.'setup'.CONF_EXT,'.php' == CONF_EXT ? "<?php\nreturn array(\n\t//'配置项'=>'配置值'\n);":'');
            // 写入模块配置文件
            if(!is_file(APP_PATH.$module.'/Conf/setup'.CONF_EXT))
                file_put_contents(APP_PATH.$module.'/Conf/setup'.CONF_EXT,'.php' == CONF_EXT ? "<?php\nreturn array(\n\t//'配置项'=>'配置值'\n);":'');
            // 生成模块的测试控制器
    ...
    static public function buildDirSecure($dirs=array()) {
        ...
        if(BUILD_DIR_SECURE) {
            defined('DIR_SECURE_FILENAME')  or define('DIR_SECURE_FILENAME',    'details.html');
