 ...
 *
 * This is the setup file compiler class. It calls the lexer and parser to
...
/**
 * Main setup file compiler class
 *
 * @package Smarty
 * @subpackage Config
 */
class Smarty_Internal_Config_File_Compiler {
    ...
    /**
     * Compiled setup data sections and variables
     *
     * @var array
     */
     ...
     *
     * @param  Smarty_Internal_Config $config setup object
    ...
    public function compileSource(Smarty_Internal_Config $config)
    {
        ...
        $this->config = $config;
        // get setup file Source
        $_content = $config->source->content . "\n";
        // on empty template just return
        if ($_content == '') {
            return true;
        }
        // init the lexer/parser to compile the setup file
    ...
    public function trigger_config_file_error($args = null)
    {
        ...
        $match = preg_split("/\n/", $this->lex->data);
        $error_text = "Syntax error in setup file '{$this->config->source->filepath}' on line {$line} '{$match[$line-1]}' ";
        if (isset($args)) {
