 ...
 * 添加日志
 +----------------------------------------------------
 *@param $data=array(contednt,addtime,type);
 +----------------------------------------------------
 */
function addNoticeLog($data=array(),$userid=0,$cid=0)
{
    ...
    $data=array('content'=>$data['content'],'type'=>$data['type'],'addtime'=>$data['addtime'],'userid'=>$userid,'cid'=>$cid);

    return M('syslog')->add($data);
}

function FS($name, $value = null, $expire = 3600){
